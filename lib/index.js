...
function validateDraft(name, instructions) {
  const cleanName = String(name || '').trim()
  const cleanInstructions = String(instructions || '')
  if (cleanName === '') throw new Error(`${PACKAGE}: style name is required`)
  if (cleanName.length > MAX_STYLE_NAME_LENGTH) {
    throw new Error(`${PACKAGE}: style name exceeds ${MAX_STYLE_NAME_LENGTH} characters`)
  }
  if (cleanInstructions.trim() === '') throw new Error(`${PACKAGE}: style instructions are required`)
  if (cleanInstructions.length > MAX_STYLE_INSTRUCTIONS_LENGTH) {
    throw new Error(`${PACKAGE}: style instructions exceed ${MAX_STYLE_INSTRUCTIONS_LENGTH} characters`)
  }
  return { name: cleanName, instructions: cleanInstructions }
}
...
export function apply(ctx) {
  ...
  const controller = {
    ...
    async save(styleId, name, instructions) {
      const draft = validateDraft(name, instructions)
      return mutate(async () => {
